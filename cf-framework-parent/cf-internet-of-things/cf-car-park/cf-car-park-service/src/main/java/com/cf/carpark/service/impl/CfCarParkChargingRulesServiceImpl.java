...
//        }
    ...
    public CfCarParkOrder calculateTheAmounPayable(CfCarParkUseLog cfCarParkUseLog, String uid, String queryMode) throws Exception {
        ...
        if(queryMode.equals(FeeQueryMode.QUERY_MODE_QUERY_AND_UPDATE) && totalCollectionFee.compareTo(BigDecimal.ZERO)>0 && cfCarParkOrder.getCfOrder().getStatus()==PayStatus.TO_BE_PAID){
        ...
        if(queryMode.equals(FeeQueryMode.QUERY_MODE_QUERY_AND_UPDATE) && cfCarParkUseLog.getPayTime()>0 && cfCarParkOrder.getCfOrder().getStatus()==PayStatus.TO_BE_PAID && cfCarParkOrder.getCfOrder().getCollectionAmount().compareTo(BigDecimal.ZERO)==0){
        ...
        if(System.currentTimeMillis()-cfCarParkOrder.getCfOrder().getManualOfferSetTime()<=900000 && cfCarParkOrder.getCfOrder().getManualOffer().compareTo(BigDecimal.ZERO)>0){
        ...
        if(queryMode.equals(FeeQueryMode.QUERY_MODE_QUERY_AND_UPDATE) && cfCarParkOrder.getCfOrder().getPayTime()>0 && cfCarParkOrder.getCfOrder().getStatus()==PayStatus.TO_BE_PAID && cfCarParkOrder.getCfOrder().getCollectionAmount().compareTo(BigDecimal.ZERO)==0){
            cfCarParkOrder.getCfOrder().setStatus(PayStatus.PAID);
    ...
    private CfCarParkOrder calculateTheAmounPayableWithOutOther(CfCarParkUseLog cfCarParkUseLog, String uid) throws Exception{
        ...
        if(cfCarParkUseLog.getCfCarParkCarLimit()!=null){
            ...
            if(cfCarParkUseLog.getCfCarParkCarLimit().getFeeUpperLimit()!=null && cfCarParkUseLog.getCfCarParkCarLimit().getFeeUpperLimit().compareTo(BigDecimal.ZERO)>0){
...
//                if((cfCarParkUseLog.getOutTime() - cfCarParkUseLog.getInTime()>cfCarPark.getFreeTime()) && orderList!=null && orderList.size()>0 && previousCarParkUseLog!=null && (((cfCarParkUseLog.getOutTime()-previousCarParkUseLog.getInTime())<=86400000l) || ((cfCarParkUseLog.getOutTime()-previousCarParkUseLog.getInTime())>86400000l && (previousCarParkUseLog.getOutTime()-previousCarParkUseLog.getInTime())>86400000l && (cfCarParkUseLog.getOutTime()-previousCarParkUseLog.getOutTime())<=86400000l))){
                if((cfCarParkUseLog.getOutTime() - cfCarParkUseLog.getInTime()>cfCarPark.getFreeTime()) && orderList!=null && orderList.size()>0 && previousCarParkUseLog!=null){
                    ...
                    if(cfCarPark.getFeeUpperLimit()!=null && cfCarPark.getFeeUpperLimit().compareTo(BigDecimal.ZERO)>0 && orderList.get(orderList.size()-1).getAmountsPayable().compareTo(cfCarPark.getFeeUpperLimit())<0 && newEstStartTime>cfCarParkUseLog.getInTime()){
                        //本次24小时内，并且收费未达到费用上限
                        newEstStartTime = cfCarParkUseLog.getInTime();
                    ...
                    if(newEstStartTime<cfCarParkUseLog.getOutTime() && (cfCarParkUseLog.getOutTime() - newEstStartTime>cfCarPark.getFreeTime())){
                        ...
                        if(cfCarPark.getFeeUpperLimit()!=null && cfCarPark.getFeeUpperLimit().compareTo(BigDecimal.ZERO)>0 && totalPaid.compareTo(cfCarPark.getFeeUpperLimit())<0){
        ...
        //如果费用为0则不存在代付，不标记代付
        if(cfOrder.getAmountsPayable().compareTo(BigDecimal.ZERO)==0 && cfOrder.getStatus()==PayStatus.TO_BE_PAID){
...
//        return scannedChargeback(cfOrder, cfUserPaymentAgency, ccbScannedChargebackForm);
    ...
    public CfOrder scannedChargeback(CfOrder cfOrder, CfUserPaymentAgency cfUserPaymentAgency, CCBScannedChargebackForm ccbScannedChargebackForm) throws Exception {
        if(cfOrder.getAmountsPayable().compareTo(BigDecimal.ZERO)==0){
            return cfOrder;
        }
